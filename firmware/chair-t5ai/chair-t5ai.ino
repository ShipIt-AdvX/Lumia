 ...
 *
 * 引脚：P7 / P6 / P5 / P4 开漏点动；P19 红外就座（1=有人 0=无人）
 ...
 * 就座：变化时 POST 桌宠 /api/sit {"seated":true/false}
 */
...
#endif
#ifndef IR_SAMPLE_MS
#define IR_SAMPLE_MS 200
#endif
#ifndef IR_DEBOUNCE_N
#define IR_DEBOUNCE_N 3
#endif
...
uint32_t lastIrLogMs = 0;
bool irSeated = false;
bool irSeatedInit = false;
int irStable = 0;
int irPending = -1;

static int readIrDig() { return digitalRead(PIN_IR) ? 1 : 0; }
...
/* 就座变化上报桌宠 POST /api/sit */
static bool reportSitToPet(bool seated) {
  if (petHost.isEmpty() || WiFi.status() != WSS_GOT_IP) {
    PR_NOTICE("[sit] skip report (no wifi/pet) seated=%d", seated ? 1 : 0);
    return false;
  }
  WiFiClient cli;
  cli.setTimeout(2500);
  if (!cli.connect(petHost.c_str(), petPort)) {
    PR_NOTICE("[sit] connect fail %s:%u", petHost.c_str(), (unsigned)petPort);
    return false;
  }
  String body = String("{\"seated\":") + (seated ? "true" : "false") + "}";
  cli.print(String("POST /api/sit HTTP/1.1\r\nHost: ") + petHost +
  ...
  PR_NOTICE("[sit] report seated=%d ok=%d", seated ? 1 : 0, ok ? 1 : 0);
  return ok;
}

static void pollIrSit() {
  int dig = readIrDig(); /* 1=有人 0=无人 */
  if (irPending < 0 || dig != irPending) {
    irPending = dig;
    irStable = 1;
  } else {
    irStable++;
  }
  if (irStable < IR_DEBOUNCE_N) return;

  bool seated = (irPending == 1);
  if (!irSeatedInit || seated != irSeated) {
    irSeatedInit = true;
    irSeated = seated;
    PR_NOTICE("[sit] P%d dig=%d -> %s", PIN_IR, irPending, seated ? "SEATED" : "EMPTY");
    digitalWrite(LED_PIN, seated ? HIGH : LOW);
    reportSitToPet(seated);
  }
}
...
static String pageHtml() {
  ...
  h += F("</div><button type=button class=ghost id=pulse style=\"margin-top:12px\">点动拉伸脚</button>");
  h += F("<button type=button class=ghost id=ir style=\"margin-top:8px\">读红外就座 P19</button>");
  h += F("<pre id=out class=hint style=\"white-space:pre-wrap;max-height:220px;overflow:auto\"></pre></div>");
  h += F("<script>"
         ...
         "document.getElementById('ir').onclick=async()=>{const r=await fetch('/api/ir');out.textContent=await r.text()};"
         "</script></body></html>");
  return h;
}
...
static void handleClient(WiFiClient &client) {
  ...
  if (path == "/api/ir" || path == "/api/sit_sensor") {
    int dig = readIrDig();
    String j = "{\"ok\":true,\"pin\":";
    j += String(PIN_IR);
    j += ",\"dig\":";
    j += String(dig);
    j += ",\"seated\":";
  ...
  if (path == "/api/status") {
    ...
    j += "\"ir_dig\":" + String(readIrDig()) + ",";
    j += "\"seated\":" + String(irSeated ? "true" : "false") + ",";
...
void setup() {
  ...
  {
    int dig = readIrDig();
    PR_NOTICE("portal :80  relay :8790  ir=P%d dig=%d (%s)  pins=%d/%d/%d/%d", PIN_IR, dig,
              dig ? "SEATED" : "EMPTY", PIN_P7, PIN_P6, PIN_P5, PIN_P4);
  }
...
static void pollSerialCmds() {
  ...
  while (Serial.available()) {
    ...
    if (c == '\n') {
      line.trim();
      if (line.equalsIgnoreCase("ir") || line.equalsIgnoreCase("sit")) {
        int dig = readIrDig();
        PR_NOTICE("[IR P%d] dig=%d seated=%s", PIN_IR, dig, dig ? "yes" : "no");
      } else if (line.length()) {
        PR_NOTICE("[cmd] unknown '%s' (ir|sit)", line.c_str());
      }
      line = "";
...
void loop() {
  ...
  uint32_t now = millis();
  if (now - lastIrLogMs >= IR_SAMPLE_MS) {
    lastIrLogMs = now;
    int dig = readIrDig();
    PR_NOTICE("[IR P%d] dig=%d %s", PIN_IR, dig, dig ? "SEATED" : "EMPTY");
    pollIrSit();
  }
