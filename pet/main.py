...
def main() -> int:
    ...
    # 连接电脑大脑：睡觉/吃饭/久坐导演
    brain = None
    brain_url = str(config.get("brain_url") or "").strip()
    if brain_url:
        from lumia.brain import BrainClient

        brain = BrainClient(brain_url, pet.apply_director, interval_ms=3000)
        brain.start()
        log.info("已连接大脑导演: %s", brain_url)
    else:
        log.warning("未配置 brain_url，桌宠不跟随提醒状态")
